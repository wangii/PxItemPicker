...
-(UIView*)cachedOverlay;
{
    if (!cachedOverlay) {
        if ([self.delegate respondsToSelector:@selector(selectedOverlayViewOfItemPicker:)]) {
			...
			NSString* path = [[NSBundle mainBundle] pathForResource:@"PxItemPicker.bundle/bluedot.png" 
                                                             ofType:nil];
            cachedOverlay = [[[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:path]] autorelease];
        }
